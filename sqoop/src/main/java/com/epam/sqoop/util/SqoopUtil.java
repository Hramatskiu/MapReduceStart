...
public class SqoopUtil {
    public static void describe(List<MConfig> configs, ResourceBundle resource) {
        configs.stream().collect(HashMap<String, List<MInput<?>>>::new, (m, c) -> m.put(c.getLabelKey(), c.getInputs()), (m, u) -> {})
                .forEach((k, v) -> {
                    System.out.println(k);
                    v.stream().collect(HashMap<String, Object>::new, (m, c) -> m.put(c.getLabelKey(), c.getValue()), (m, u) -> {})
