...
public class SqoopTaskApp {
  ...
  private static void readLinksValidationMessages( SqoopClient sqoopClient ) {
    sqoopClient.getLinks().stream().map( MLink::getConnectorLinkConfig ).map( MConfigList::getConfigs )
      .forEach( SqoopUtil::printValidationMessages );
  }

  private static void testConnection( SqoopClient sqoopClient ) {
    SqoopUtil.describe( sqoopClient.getConnector( "hdfs-connector" ).getLinkConfig().getConfigs(),
      sqoopClient.getConnectorConfigBundle( "hdfs-connector" ) );
    SqoopUtil.describe( sqoopClient.getConnector( "hive-connector" ).getLinkConfig().getConfigs(),
      sqoopClient.getConnectorConfigBundle( "hive-connector" ) );
    SqoopUtil.describe( sqoopClient.getConnector( "mysql-connector" ).getLinkConfig().getConfigs(),
      sqoopClient.getConnectorConfigBundle( "mysql-connector" ) );
  }
  ...
  private static MLink createFromLink( SqoopClient sqoopClient ) {
    return SqoopLinkCreator.createLink( sqoopClient, "hive-connector", "fromLink", "devuser",
      SqoopConfigsCreator.createConfigMapForJDBCConnector(
  ...
  private static MJob createJob( SqoopClient sqoopClient ) {
    return SqoopJobCreator.createJob( sqoopClient, "fromLink", "toLink", "test_job", "devuser",
      ...
      SqoopConfigsCreator.createJobOtHDFSConfig( "/user/Stanislau_Hramatskiu/sqoop_test" ),
      SqoopConfigsCreator.createJobDriverConfig( 3 ) );
  }
}
