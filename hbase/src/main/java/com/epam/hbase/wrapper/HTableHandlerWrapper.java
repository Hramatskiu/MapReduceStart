...
public class HTableHandlerWrapper {
  ...
  public List<String> getTableFamilies( String tableName ) {
    try ( HBaseConnectionWrapper hBaseConnectionWrapper = new HBaseConnectionWrapper( hBaseConnectionPool ) ) {
      return Arrays.stream( getTable( hBaseConnectionWrapper, tableName ).getTableDescriptor().getColumnFamilies() )
        .map( HColumnDescriptor::getNameAsString ).collect( Collectors.toList() );
  ...
  public NavigableMap<byte[], byte[]> getFamilyMap( String tableName, String rowName, String familyName ) {
    try ( HBaseConnectionWrapper hBaseConnectionWrapper = new HBaseConnectionWrapper( hBaseConnectionPool ) ) {
      return getFamilyMap(
        getTable( hBaseConnectionWrapper, tableName ).get( HBaseQueryHelper.createGetSentence( rowName ) ),
        familyName );
    } catch ( IOException ex ) {
      //logging
    }
  ...
  public void addFamilyToTable( String tableName, String familyName ) {
    try ( HBaseConnectionWrapper hBaseConnectionWrapper = new HBaseConnectionWrapper( hBaseConnectionPool ) ) {
      hBaseConnectionWrapper.getAdmin()
        .addColumn( TableName.valueOf( tableName ), new HColumnDescriptor( Bytes.toBytes( familyName ) ) );
      //getTable(hBaseConnectionWrapper, tableName).getTableDescriptor().addFamily(new HColumnDescriptor(Bytes
      // .toBytes(familyName)));
    } catch ( IOException ex ) {
      //logging
    }
  ...
  public List<String> getTableNamesList() {
    try ( HBaseConnectionWrapper hBaseConnectionWrapper = new HBaseConnectionWrapper( hBaseConnectionPool ) ) {
      return Arrays.stream( hBaseConnectionWrapper.getAdmin().listTableNames() ).map( TableName::toString )
  ...
  public void putDataInTable( String tableName, String rowName, String familyName, String qualifier, String value ) {
    try ( HBaseConnectionWrapper hBaseConnectionWrapper = new HBaseConnectionWrapper( hBaseConnectionPool ) ) {
      HBaseQueryHelper
        .tryToPutData( rowName, familyName, qualifier, value, getTable( hBaseConnectionWrapper, tableName ) );
    } catch ( IOException ex ) {
      //logging
    }
  }
  ...
  public ResultScanner getResultScanner( String tableName, String familyName, String qualifierName ) {
    try ( HBaseConnectionWrapper hBaseConnectionWrapper = new HBaseConnectionWrapper( hBaseConnectionPool ) ) {
      return getTable( hBaseConnectionWrapper, tableName )
        .getScanner( HBaseQueryHelper.createScanForColumn( familyName, qualifierName ) );
    } catch ( IOException ex ) {
      //logging
    }
  ...
  private boolean createTable( HBaseConnectionWrapper connection, String tableName, String familyName )
    throws IOException {
  ...
  private boolean createTable( HBaseConnectionWrapper connection, String tableName, List<String> familyNames )
    throws IOException {
