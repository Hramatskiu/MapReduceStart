...
public class HdfsTaskApp {
  ...
  private static FSDataOutputStream createFile( String filePath, boolean overwrite, FileSystem fileSystem )
    throws IOException {
  ...
  private static List<String> getDataNodesHostNames( FileSystem fileSystem ) throws IOException {
    return Arrays.stream( ( (DistributedFileSystem) fileSystem ).getDataNodeStats() ).map( DatanodeInfo::getHostName )
